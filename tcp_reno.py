...
def run(one_way_delay_ms=21):
    ...
    # Start iperf servers
    h3.cmd("iperf -s -i 1 -p 5001 > h3_server.txt &")
    h4.cmd("iperf -s -i 1 -p 5001 > h4_server.txt &")

    # Start first flow: h1 → h3
    output_file = "tcp_reno"
    log_tcp_info(h1, h3.IP(), output_file + "_flow2.txt")
    h1.cmd(f"iperf -c {h3.IP()} -t 2000 -i 1 -p 5001 > h1_client.txt")

    # Start second flow: h2 → h4 (delayed start)
    log_tcp_info(h2, h4.IP(), output_file + "_flow2.txt")
    h2.cmd(f"sleep 250; iperf -c {h4.IP()} -t 1750 -i 1 -p 5001 > h2_client.txt")
